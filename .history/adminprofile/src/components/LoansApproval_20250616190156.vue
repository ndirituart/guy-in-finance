    ...
    <div class="content-area">
      ...
      <ul v-if="pendingLoans.length">
        <li v-for="loan in pendingLoans" :key="loan.id" class="loan-item">
          ...
          <p><strong>Date:</strong> {{ new Date(loan.requestedOn).toLocaleString() }}</p>
          <div class="actions">
...
export default {
  ...
  methods: {
    ...
    loadPendingLoans() {
      // This fetches pending loans from the shared 'adminMockLoanRequests' in localStorage
    ...
    approveLoan(loanId) {
      ...
      if (loanIndex !== -1) {
        ...
        localStorage.setItem('adminMockLoanRequests', JSON.stringify(allLoans)); // Update shared mock
        this.loadPendingLoans(); // Refresh the list in this component
        this.showMessage(`Loan ${loanId} approved successfully!`, 'success');
        
        // --- Mocking User-Side Updates (Backend would handle this) ---
        // This updates the user's specific loan history mock in their localStorage
    ...
    updateUserLoanBalance(userId, loanAmount) {
        // This updates the admin's mock of all user balances
    ...
    updateUserLoanStatusInLocalStorage(userId, loanId, newStatus) {
        // This specifically updates the user's *individual* loan history
