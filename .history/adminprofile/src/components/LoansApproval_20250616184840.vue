...
export default {
  ...
  mounted() {
    ...
    // Add event listener for window focus (ensures data is fresh when returning to tab)
    window.addEventListener('focus', this.loadPendingLoans); // <--- ADDED THIS LINE
  },
  beforeUnmount() {
    // Clean up event listeners when component is unmounted
    window.removeEventListener('storage', this.handleStorageChange);
    window.removeEventListener('focus', this.loadPendingLoans); // <--- ADDED THIS LINE
  },
  methods: {
    handleStorageChange(event) {
      // Check if the change is relevant to our admin mock loans
      if (event.key === 'adminMockLoanRequests') {
        console.log('Storage event for adminMockLoanRequests detected. Reloading loans.'); // For debugging
