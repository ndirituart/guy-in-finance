...
const handleLogin = async () => {
  ...
  try {
    const response = await axios.post('http://127.0.0.1:8000/api/login', {
      email: email.value,    // Send email field
      password: password.value, //send password field
    });

    console.log('Login successful:', response.data);

    // Assuming your Laravel backend returns a 'token' on successful login
    const token = response.data.token;
    if (token) {
      // Store the token (e.g., in localStorage for persistent login)
      localStorage.setItem('authToken', token);

      // Set Axios default header for all future requests that need authentication
      axios.defaults.headers.common['Authorization'] = `Bearer ${token}`;

      // Redirect to home page when user logs in successfully
      
    } else {
      // This case might happen if the backend responds 200 but no token is present
    ...
    if (error.response && error.response.status === 401) {
      ...
      errorMessage.value = error.response.data.errors.email || 'Invalid credentials. Please try again.';
    } else if (error.response && error.response.status === 422) {
      // Validation errors (e.g., email or password missing)
