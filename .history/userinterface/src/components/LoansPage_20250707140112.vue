...
export default {
  ...
  methods: {
    async submitAppLoanRequest() {
      ...
      try {
        // --- Calling the backend for loan requests ---
        const response = await axios.post('/api/loan-requests', newRequest);
        // eslint-disable-next-line no-unused-vars for now
       const submittedLoan = response.data; 

        // the admin functions for mock requests before we automate this
        const mockRequestId = 'LR' + Date.now();
        const simulatedSubmittedLoan = {
          id: mockRequestId,
          ...newRequest, // spread userId and amount
          status: 'pending',
          requestedOn: new Date().toISOString(),
          lastUpdated: new Date().toISOString(),
        };

        // Simulate saving to 'admin's database'
