...
export default {
  ...
  mounted() {
    // On mount, load the user's *individual* loan history from localStorage
    this.loadUserLoanHistory();
    // Also check the status (which relies on shared adminMockLoanRequests)
  ...
  methods: {
    async submitAppLoanRequest() {
      ...
      // --- CRITICAL MOCKING FOR ADMIN VISIBILITY ---
      // This sends the request to the 'admin's mock database' in localStorage
      let adminLoansMock = JSON.parse(localStorage.getItem('adminMockLoanRequests') || '[]');
      adminLoansMock.unshift(newRequest); // Add to the top for easy viewing
      localStorage.setItem('adminMockLoanRequests', JSON.stringify(adminLoansMock));

      // --- Also update the user's *own* local loan history ---
      let userLoansMock = JSON.parse(localStorage.getItem(`userLoans_${this.currentUserId}`) || '[]');
      userLoansMock.unshift(newRequest); // Add to user's history
      localStorage.setItem(`userLoans_${this.currentUserId}`, JSON.stringify(userLoansMock));

      // Update the component's data from the *user's* local storage
      this.loadUserLoanHistory(); 
    ...
    async checkLoanStatus() {
      // This method fetches the status of *this user's* loans from the mock backend (localStorage)
      ...
      if (this.loanStatusHistory.length > 0) {
          ...
          if (latestLoan.status === 'approved') {
              ...
              // In a real app, you'd then trigger a balance update
    ...
    loadUserLoanHistory() {
      // Helper to load user's specific loan history
