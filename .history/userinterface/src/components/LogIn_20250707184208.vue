...
const handleLogin = async () => {
  ...
  try {
    // CORRECTED AXIOS.POST SYNTAX:
    // axios.post(url, data, config)
    const response = await axios.post('http://127.0.0.1:5000/api/login',
      { // This is the 'data' object (second argument)
        email: email.value,
        password: password.value,
      },
      { // This is the 'config' object (third argument)
    ...
    if (token) {
      ...
      router.push({ name: 'Home' }); // Assuming your home page route name is 'Home'
    } else {
