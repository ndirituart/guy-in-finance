...
const rent = ref({
  paymentType: 'paybill', // 'paybill' or 'till'
  paybillNumber: '',
  accountNumber: '', // Used for both Paybill Acc No and Till No
  amount: null,
  saveDetails: false,
});

const security = ref({
  paymentType: 'paybill', // 'paybill', 'till', or 'sendmoney'
  paybillNumber: '',
  accountNumber: '', // Used for Paybill Acc No, Till No, or Phone No
  amount: null,
  saveDetails: false,
});
...
const loadSavedDetails = (key, stateRef) => {
  ...
  if (saved) {
    try {
      ...
      // Merge saved properties, but don't overwrite `amount` or `saveDetails`
      // unless specifically intended to load last paid amount
      for (const prop in parsed) {
        if (prop !== 'amount' && prop !== 'saveDetails' && stateRef.value.hasOwnProperty(prop)) {
          stateRef.value[prop] = parsed[prop];
        }
      }
      // Set saveDetails to true if details were loaded
      stateRef.value.saveDetails = true;
    } catch (e) {
...
const saveDetails = (key, data) => {
  if (data.saveDetails) {
    // Only save the relevant input fields, not amount or saveDetails flag itself
    const dataToSave = {};
    for (const prop in data) {
      if (prop !== 'amount' && prop !== 'saveDetails') {
...
// Watchers to save/clear details when checkbox is toggled
...
const processTransaction = (amount, billType, billNumber, accountNumber) => {
  ...
  if (accountBalance.value >= amount) {
    ...
    transactionMessage.value = `${billType} Payment Successful! New Balance: Ksh. ${accountBalance.value}`;
...
const validatePhoneNumber = (number) => /^\+254\d{9}$/.test(number); // Assuming +254 format
...
const processWaterPayment = () => {
  ...
  processTransaction(water.value.amount, 'Water Bill', '444400', water.value.accountNumber);
...
const processKPLCPayment = () => {
  ...
  processTransaction(kplc.value.amount, 'KPLC Bill', '88888', kplc.value.accountNumber);
...
const processRentPayment = () => {
  if (rent.value.paymentType === 'paybill') {
    ...
    if (!validateShortNumber(rent.value.accountNumber)) { // accountNumber is acting as tillNumber here
  ...
  processTransaction(rent.value.amount, 'Rent Bill', rent.value.paybillNumber || rent.value.accountNumber, rent.value.accountNumber);
...
const processSecurityPayment = () => {
  ...
  if (security.value.paymentType === 'paybill') {
    ...
    billNumber = security.value.accountNumber; // Till number is the main identifier
    ...
    billNumber = security.value.accountNumber; // Phone number is the main identifier
  } else {
  ...
  processTransaction(security.value.amount, 'Security Bill', billNumber, accNum);
...
const processGarbagePayment = () => {
  ...
  processTransaction(garbage.value.amount, 'Garbage Collection Bill', billNumber, accNum);
...
const processHELBPrompt = () => {
  ...
  transactionFailed.value = false; // This is not a failed transaction, just a prompt
};
...
const handleTransactionFailure = () => {
  ...
  // Implement actual loan logic here
  // For now, just reset the failed state after a delay
  setTimeout(() => {
    transactionFailed.value = false;
...
.transaction-actions button:first-child {
  background-color: #ffc107; /* Yellow for loan */
  color: #333;
}
