...
export default {
  ...
  setup() {
    const profile = ref({
      ...
      newPassword: '', // In case user needs to create a new password
      confirmNewPassword: '',
    });
    ...
    const formValid = computed(() => {
      // Validation for password length, phone number format and age >=14
      return (
        !newPasswordMismatch.value &&
        age.value >= 14 && 
        profile.value.phone.startsWith('+254') &&
        (profile.value.newPassword === '' || profile.value.newPassword.length >= 6) 
      );
    });
    ...
    onMounted(async () => {
      try {
        ...
        profile.value.birthday = userData.birthday; //(YYYY-MM-DD)
    ...
    const updateProfile = async () => {
      ...
      try {
        ...
        formData.append('middle_name', profile.value.middleName || ''); // undefined
        ...
        // Append new password if need be
