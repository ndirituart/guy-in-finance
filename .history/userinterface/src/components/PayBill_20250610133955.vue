...
export default {
  ...
  methods: {
    processTillPayment() {
      if (!this.validateTillNumber(this.tillNumber) || this.amountTill <= 0 || !this.validatePin(this.pinTill)) {
        alert('Please enter a valid Till Number, a positive amount, and the correct PIN.');
        return;
      }
    ...
    processPaybillPayment() {
      if (!this.validatePaybillNumber(this.paybillNumber) || !this.validateAccountNumber(this.accountNumber) || this.amountPaybill <= 0 || !this.validatePin(this.pinPaybill)) {
        alert('Please enter valid Paybill/Account Numbers, a positive amount, and the correct PIN.');
        return;
      }
