...
export default {
  ...
  setup() {
    ...
    const goToLogin = () => {
      router.push('/login'); // Assuming your login route path is '/login'
    };
    ...
    onMounted(async () => {
      try {
        ...
        if (!token) {
          // In case no AUTH token is found, send to login page
        ...
        // Redirect to login on API fetch error (e.g., token expired, invalid)
        router.push('/login'); 
      }
    });
    ...
    const updateProfile = async () => {
      ...
      try {
        ...
        // Optionally (when user gives a Profile Pic)
    ...
    return {
      ...
      handleImageUpload,
      updateProfile,
      goToLogin, // Make goToLogin available in the template
    };
  },
};
</script>
