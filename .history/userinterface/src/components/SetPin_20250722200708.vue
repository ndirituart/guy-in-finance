<!-- I think just like M-PESA it is necessary to have users add a 4 digit PIN for transactions -->
<template>
...
export default {
  ...
  setup() {
    ...
    if (!userId) {
      ...
      // To lead new users to signup page to register
      ...
      if (currentStep.value === 1) {
        ...
        if (confirmPinInput.value.length === 4) {
          if (pinInput.value === confirmPinInput.value) {
            // PINs must match before proceeding
